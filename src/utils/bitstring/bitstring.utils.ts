 ...
 */
export function setStatusAtIndex(
  ...
  // This prevents (8 - bitOffset - statusSize) from becoming negative when statusSize + bitOffset > 8
  // Example: if bitOffset=6 and statusSize=4, we can only fit 2 bits in the first byte
  const firstPartBits = Math.min(statusSize, 8 - bitOffset);

  // Create mask for the bits we're setting in the first byte
  // Shift left to position the mask at the correct bit offset
  const firstPartMask =
    ((1 << firstPartBits) - 1) << (8 - bitOffset - firstPartBits);

  // Clear and set bits in the first byte
  // First clear the target bits using inverted mask, then set new value
...
export function getStatusAtIndex(
  ...
  if (remainingBits > 0 && byteIndex + 1 < bitstring.length) {
    ...
    const nextByteValue =
      (bitstring[byteIndex + 1] >> nextByteShift) & ((1 << remainingBits) - 1);
...
/**
 * Validates base64url format (RFC 4648 Section 5)
 * @param value The string to validate
 * @returns True if valid base64url format
 */
export function isValidBase64url(value: string): boolean {
  // Base64url uses A-Z, a-z, 0-9, -, _ characters only (no padding)
...
export function countSetEntries(
  ...
  for (let i = 0; i < capacity; i++) {
    const value = getStatusAtIndex(bitstring, i, statusSize);
    if (value !== 0) {
      setCount++;
    }
  }
