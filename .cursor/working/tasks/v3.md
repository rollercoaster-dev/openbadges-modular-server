...
- [x] Task 1.2.1: Conduct detailed research on the StatusList2021 specification and implementation requirements.
  **Implementation Plan:**
  - Research W3C StatusList2021 specification (https://w3c.github.io/vc-status-list-2021/)
  - Understand bitstring format (16k entries, base64url encoded, gzip compressed)
  - Review credentialStatus structure and statusPurpose values
...
- [x] Task 1.2.2: Design the data model for storing StatusList2021 information in the database.
  **Implementation Plan:**
  - Create `status_lists` table: id, issuer_id, purpose, bitstring, created_at, updated_at
  - Create `credential_status_entries` table: credential_id, status_list_id, index, status, updated_at
  - Add indexes for performance (issuer_id, purpose, credential_id)
...
- [x] Task 1.2.3: Write database migration script to add necessary tables and fields for StatusList2021.
  **Implementation Plan:**
  - Create migration file: `src/infrastructure/database/migrations/add-status-list-tables.ts`
...
- [x] Task 1.2.4: Create the StatusList Service class (`src/core/status-list.service.ts`).
  **Implementation Plan:**
  ...
  - Create types file: `src/core/types/status-list.types.ts`
  - Implement basic CRUD operations for status lists

- [x] Task 1.2.5: Implement the core logic within StatusListService for generating StatusList2021 bitstrings.
  **Implementation Plan:**
  - Implement bitstring generation (16,384 bits = 2,048 bytes)
  - Add gzip compression and base64url encoding
  - Create methods: createStatusList(), generateBitstring(), compressBitstring()
  - Handle different status purposes (revocation, suspension)
...
- [x] Task 1.2.6: Implement the logic within StatusListService for updating a credential's status within its corresponding bitstring.
  **Implementation Plan:**
  - Implement updateCredentialStatus() method
  - Add bitstring manipulation (set/unset bits at specific indexes)
  - Handle concurrent updates with proper locking
  - Update compressed bitstring after changes

- [x] Task 1.2.7: Add the API endpoint `GET /v3/status-lists/:id` to retrieve a specific status list.
  **Implementation Plan:**
  - Create `src/api/controllers/status-list.controller.ts`
  - Implement getStatusList() method
  - Return proper StatusList2021 JSON-LD format
  - Add error handling and validation
  - Update `src/api/api.router.ts` with new route

- [x] Task 1.2.8: Add the API endpoint `POST /v3/credentials/:id/status` to update the status of a specific credential.
  **Implementation Plan:**
  - Add updateCredentialStatus() method to controller
  - Validate request body (status, reason)
  - Update both status list and credential record
  - Return updated credential with new credentialStatus
...
- [x] Task 1.2.9: Integrate the StatusListService with the Assertion entity and serialization process to include `credentialStatus`.
  **Implementation Plan:**
  - Modify `src/domains/assertion/assertion.entity.ts`
  - Replace placeholder credentialStatus with real StatusList2021 entries
  - Update assertion creation to assign status list index
  - Modify serialization to include proper credentialStatus URLs
  - Update repository to handle status list relationships

- [ ] Task 1.2.10: Add unit tests for the StatusListService logic (bitstring generation, status updates).
  **Implementation Plan:**
  - Create `tests/unit/core/status-list.service.test.ts`
  - Test bitstring generation and compression
  - Test status updates and bit manipulation
  - Test error conditions and edge cases
  - Mock repository dependencies

- [ ] Task 1.2.11: Add API tests for the new status list endpoints (`GET` and `POST`).
  **Implementation Plan:**
  - Create `tests/integration/api/status-list.test.ts`
  - Test GET /v3/status-lists/:id endpoint
  - Test POST /v3/credentials/:id/status endpoint
...
- [ ] Task 1.2.12: Add E2E tests to verify the end-to-end functionality of status list creation and status updates.
  **Implementation Plan:**
  - Create `tests/e2e/status-list.e2e.test.ts`
  - Test complete workflow: create credential → update status → verify status
  - Test status list retrieval and format compliance
  - Test integration with assertion verification
  - Test performance with large status lists
