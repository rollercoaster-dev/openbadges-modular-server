...
- [ ] Task 2.2.1: Design the API endpoints for batch operations (`POST /v3/credentials/batch`, `GET /v3/credentials/batch?ids=...`, `PUT /v3/credentials/batch/status`).
- [ ] Task 2.2.2: Implement the batch credential creation endpoint (`POST /v3/credentials/batch`).
...
- [ ] Task 3.2.1: Research the `credentialSchema` property in Open Badges v3.0 and its role in validation.
  **COMPLETED** - Research shows:
  - `credentialSchema` is an optional array property in VerifiableCredential
  - References external JSON schemas for credential validation
  - Common type: `1EdTechJsonSchemaValidator2019`
  - Used for validating credential structure beyond basic OB3.0 requirements
  - Example: `{"id": "https://purl.imsglobal.org/spec/ob/v3p0/schema/json/ob_v3p0_achievementcredential_schema.json", "type": "1EdTechJsonSchemaValidator2019"}`

- [x] Task 3.2.2: Identify and evaluate a suitable JSON Schema validation library for the project.
  **COMPLETED** - Selected `ajv` with `ajv-formats`:
  - Excellent TypeScript support with @types/ajv
  - Supports JSON Schema draft-07, 2019-09, and 2020-12
  - High performance with compilation and caching
  - Async validation support for remote schemas
  - Installed: ajv@8.x, ajv-formats, @types/ajv

- [x] Task 3.2.3: Integrate the chosen JSON Schema validation library into the project.
  **COMPLETED**:
  - Installed ajv and ajv-formats packages
  - Created `src/utils/json-schema/ajv-config.ts` with:
    - Configured AJV instance with appropriate options
    - Added custom formats for IRI and datetime
    - Implemented schema caching and remote loading
    - Added validation function compilation and caching

- [x] Task 3.2.4: Create a dedicated Schema Validation Service.
  **COMPLETED**:
  - Created `src/utils/errors/schema-validation.errors.ts` with comprehensive error types
  - Created `src/core/schema-validation.service.ts` with:
    - Injectable NestJS service for schema validation
    - Support for multiple credential schemas per credential
    - Schema caching and validation function caching
    - Timeout handling for schema fetching
    - Comprehensive error handling and logging
    - Support for 1EdTechJsonSchemaValidator2019 and JsonSchemaValidator2020

- [x] Task 3.2.5: Implement the logic within the Schema Validation Service for validating credentials against external JSON schemas referenced by `credentialSchema`.
  **COMPLETED**:
  - Implemented schema fetching from URLs with caching and timeout handling
  - Added credential validation against fetched schemas using AJV
  - Comprehensive error handling for network failures and invalid schemas
  - Support for multiple schema types with extensible enum
  - Validation against multiple schemas per credential
  - Detailed error reporting with validation failure details

- [x] Task 3.2.6: Implement logic for applying custom validation rules beyond the standard schema if required.
  **COMPLETED**:
  - Added extensible custom validation rule system with CustomValidationRule type
  - Implemented ValidationContext and ValidationResult interfaces
  - Added applyCustomValidationRules method with timeout and error handling
  - Created predefined custom rules: validateIssuanceDate, validateExpirationDate, validateIssuer
  - Custom rules are applied after schema validation and support async operations

- [x] Task 3.2.7: Integrate the Schema Validation Service into the Assertion creation and update flow to perform validation.
  **COMPLETED**:
  - Updated Assertion entity to include credentialSchema property
  - Added credentialSchema to both OB3 toObject() and toJsonLd() methods
  - Updated Zod validation schemas with CredentialSchemaSchema
  - Added CredentialSchemaDto to DTOs
  - Modified assertion controller to inject SchemaValidationService
  - Integrated schema validation in both createAssertion and updateAssertion methods
  - Updated database schemas (PostgreSQL and SQLite) to store credentialSchema
  - Applied predefined custom validation rules during schema validation

- [x] Task 3.2.8: Add unit tests for the schema validation logic using various valid and invalid schemas/credentials.
  **COMPLETED**:
  - Created comprehensive unit tests in `tests/core/schema-validation.service.test.ts`
  - Added test fixture schemas: valid-achievement-credential-schema.json, invalid-schema.json, strict-credential-schema.json
  - Tested schema fetching, caching, and validation function compilation
  - Tested credential validation success/failure cases with detailed error checking
  - Tested custom validation rules (issuanceDate, expirationDate, issuer)
  - Tested error handling for network failures, timeouts, and invalid schemas
  - Added integration tests with real Open Badges 3.0 schema fixtures
  - Tested cache management and clearing functionality

- [x] Task 3.2.9: Add API tests to verify that invalid credentials are rejected with appropriate error messages due to schema validation failures.
  **COMPLETED**:
  - Created comprehensive API tests in `tests/api/assertion-schema-validation.test.ts`
  - Tested assertion creation with valid and invalid credentialSchema
  - Tested proper error responses with BadRequestError for schema validation failures
  - Verified error message clarity for various failure scenarios:
    - Unsupported schema types
    - Schema fetch failures (404, network errors)
    - Credential validation failures against schema
    - Invalid schema format errors
    - Custom validation rule failures (future dates, missing fields)
    - Schema validation timeouts
  - Tested assertion update scenarios with schema validation
  - All tests verify appropriate error messages are returned to API consumers
