...
for (const tableName of maliciousTableNames) {
  try {
    ...
    if (
      ...
      logger.info(`✅ Successfully blocked: "${tableName}"`);
      successfulBlocks++;
    } else {
      logger.warn(
...
for (const tableName of validTableNames) {
  try {
    logger.info(`Testing: "${tableName}"`);
    validateDatabaseName(tableName);
    logger.info(`✅ Valid table name passed: "${tableName}"`);
    validPassed++;
  } catch (error) {
    logger.error(
...
if (failedAttempts === 0 && validFailed === 0) {
  ...
  logger.info('✅ All malicious attempts were blocked');
  logger.info('✅ All valid table names were accepted');
} else {
