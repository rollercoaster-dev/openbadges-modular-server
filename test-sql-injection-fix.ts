...
async function testSQLInjectionProtection(): Promise<TestResult> {
  ...
  if (!isAvailable) {
    ...
    return {
      success: true,
      message: 'Test skipped - PostgreSQL not available',
      details: {
        totalAttempts: 0,
        successfulBlocks: 0,
        failedAttempts: 0,
  ...
  try {
    ...
    if (failedAttempts === 0) {
      ...
      return {
        success: true,
        message: 'All SQL injection attempts were successfully blocked',
        details: {
          totalAttempts: maliciousTableNames.length,
          successfulBlocks,
          failedAttempts,
        },
      };
      ...
      return {
        success: false,
        message: `Security vulnerability detected: ${failedAttempts} malicious attempts were not blocked`,
        details: {
          totalAttempts: maliciousTableNames.length,
          successfulBlocks,
          failedAttempts,
...
testSQLInjectionProtection()
  .then((result) => {
    if (result.success) {
      logger.info(`✅ Test completed successfully: ${result.message}`);
  ...
  .catch((error) => {
    ...
    process.exit(2); // Exit with different code for execution errors
  });
